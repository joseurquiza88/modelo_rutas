...
exposicion_total <- function (lista_viaje,tiempo_actividad, modo, concentraciones_grilla,key,seleccion,salida_exp,
  ...
  for (i in 1:length(lista_viaje$long)){
    ...
    grilla <- busqueda_grilla(hora_inicio=hora_llegada,hora_fin=hora_prox_salida ,directorio_grilla,formato_hora="%Y-%m-%d %H:%M:%S")
    ...
    if (i==1){
      ...
      grilla_origen <- busqueda_grilla(hora_inicio=hora_inicial, hora_fin=horario[1],directorio_grilla,formato_hora="%Y-%m-%d %H:%M:%S")
      punto <- st_read("./temp/temp_punto.shp",quiet = TRUE)
