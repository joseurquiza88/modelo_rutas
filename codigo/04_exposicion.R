...
exposicion_total <- function (lista_viaje,tiempo_actividad, modo, grilla,key,seleccion,salida_exp,
  ...
  for (i in 1:length(lista_viaje$long)){
    ...
    if (i == length(lista_viaje$long)){
      ...
      ruta_seleccionada <-  alternativas_recorridos (origen=origen_coords,destino=origen_coords_1,modo=modo[i], grilla,key=key_1,salida = "df",horario =horario[i])
      }else {
      origen_coords <- paste((lista_viaje[i,2]),(lista_viaje[i,1]),sep = ",")
      destino_coords <- paste((lista_viaje[i+1,2]),(lista_viaje[i+1,1]),sep = ",")
        
      ruta_seleccionada <-  alternativas_recorridos (origen=origen_coords,destino=destino_coords,modo=modo[i], grilla,key=key_2,salida = "df",horario = horario[i])
